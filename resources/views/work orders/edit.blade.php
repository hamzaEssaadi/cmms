...
@section('title')Edit a Work order @endsection
@section('css')
...
@section('location-nav')
    <ol class="breadcrumb panel">
        ...
        <li class="active"><strong>Edit a Work order</strong></li>
    </ol>
@endsection
...
@section('content')
    <div class="row">
        <div class="col-md-6 col-md-offset-3 col-xs-12">
            ...
            @endif
            <form method="post" action="{{route('work-orders.update',['workOrder'=>$workOrder->id])}}">
                <div class="form-group  @if($errors->get('code')) has-error @endif ">
                    <label for="code">Code :</label>
                    <input id="code" type="text" class="form-control" value="{{old('code',$workOrder->code)}}" name="code" required=""/>
                ...
                <div class="form-group  @if($errors->get('date')) has-error @endif ">
                    ...
                    <input id="date" type="text" class="form-control mydate" value="{{old('date',$workOrder->demand_at)}}" name="date" required=""/>
                ...
                <div class="form-group">
                    ...
                    <select id="type" name="type" class="form-control" required="">
                        @foreach($types as $type)
                            <option @if($type->id==$workOrder->work_order_type_id) selected="" @endif value="{{$type->id}}">{{$type->code}}</option>
                        @endforeach
                    </select>
                </div>
                <div class="form-group">
                    <label for="billable">Billable :</label>
                    <input id="billable" @if($workOrder->billable) checked="" @endif name="billable" type="checkbox" class="js-switch"/>
                </div>
                <div class="form-group">
                    <label for="cost">Estimated cost (DH) :</label>
                    <input id="cost" name="cost" value="{{old('cost',$workOrder->cost)}}" type="number" step="0.01"  required="" class="form-control"/>
                </div>
